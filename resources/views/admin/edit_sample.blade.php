...
<div class="row">
 	<div class="col-sm-8 offset-sm-2">
    	<h3 class="display-5">{{__('lang.edit-sample-for-order-x', ['order_id' => $sample->kit->order->id])}}</h3>
      	<div>
            @if ($errors->any())
        	...
        	<form method="post" action="{{action('SampleController@update', ['id' => $sample->id])}}">
            	...
            	<button type="submit" class="btn btn-primary">{{__('lang.Edit Sample')}}</button>
            	<a class="btn btn-secondary" href="{{url('/admin/samples')}}" role="button">{{__('lang.Cancel')}}</a>
