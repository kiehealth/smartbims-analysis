...
class KitsImport implements ToCollection, WithHeadingRow, SkipsOnFailure, WithMapping
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($data as $key => $val){
            ...
            $messages["$key.order_id.unique"] = "Error on row: <strong>".($key+2)."</strong>. The order_id <strong>".$val['order_id'].
            ...
            $messages["$key.kit_dispatched_date.date"] = "Error on row: <strong>".($key+2).
                                                         "</strong>. The kit_dispatched_date <strong>".$val['kit_dispatched_date'].
